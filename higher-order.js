...
let evenNumbers = mixedNumbers.filter(even=>{return even%2===0}) // = mixedNumbers.filter(/* Provide Your Callback Here */)
...
let postTaxPrices =prices.map(price=>{return price*1.07}) // = prices.map(/* Provide Your Callback Here );
...
let totalPopulation = populations.reduce((pop,lat)=> {return pop + lat})  //  = populations.reduce(/* Provide Your Callback Here */)
...
let myStrongest = monstersInYourPocket.filter(power=>power.CP>200) // Your code here
...
let ordersTotal = orders.map( order => order.totalPrice = orders.price + (orders.price*orders.tax)) // Your code here
...
let bobsTotal = purchases.reduce((tot,curr)=>{
    if(curr.owner === "Bob"){
